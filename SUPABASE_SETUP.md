...
-- ============================================
-- HELPER FUNCTION TO CHECK ADMIN (PREVENTS RECURSION)
-- ============================================
...
DROP POLICY IF EXISTS reviews_customer_create ON public.reviews;
...
-- ============================================
-- UPDATED_AT TRIGGER (SINGLE SOURCE OF TRUTH)
-- ============================================
...
DECLARE r RECORD;
BEGIN
FOR r IN SELECT tablename FROM pg*tables WHERE schemaname = 'public'
AND tablename IN ('profiles','categories','services','orders','reviews')
LOOP
EXECUTE format('DROP TRIGGER IF EXISTS trg*%I*updated_at ON public.%I', r.tablename, r.tablename);
EXECUTE format(
'CREATE TRIGGER trg*%I_updated_at BEFORE UPDATE ON public.%I
